...
const ReportsModule = (function() {
    ...
    // ==========================================
    // RENDER REPORTS (router to active tab)
    // ==========================================
    function render() {
    ...
    function renderSummary() {
        ...
        document.getElementById('reportSummaryStats').innerHTML = `
            ...
            <div class="report-stat-item">
                <div class="report-stat-value">${app.formatCurrency(totalValue)}</div>
    ...
    // ==========================================
    // LOW STOCK REPORT (no SKU column)
    // ==========================================
    ...
    function renderSuppliers() {
        ...
        const supplierData = suppliers.map(s => {
            ...
            // Transactions are stored oldest-first (Firebase push keys are
            // chronological), so the most recent delivery is the one with
            // the latest date/time, not simply the first array entry.
    ...
    function renderBranches() {
        ...
        document.getElementById('reportBranchStats').innerHTML = branchData.map(b => `
            <div class="report-stat-item">
                <div class="report-stat-value">${app.formatCurrency(b.totalValue)}</div>
                <div class="report-stat-label">${b.name}</div>
            </div>
        `).join('') + `
            <div class="report-stat-item">
                <div class="report-stat-value" style="color: var(--primary);">${app.formatCurrency(grandTotalVal)}</div>
    ...
    function renderMostUsed() {
        ...
        // "Most Used" should reflect items leaving/consumed from stock, not
        // stock coming IN. Receive transactions are inbound (supplier
        // deliveries), so counting them here previously inflated the report
        // with restocking instead of actual usage/depletion.
