...
const SettingsModule = (function() {
    ...
    function save() {
        ...
        DB.settings.save(settings).then(() => {
            ...
            app.setThemePreference(settings.theme);
    ...
    function importData(input) {
        ...
        reader.onload = e => {
            try {
                ...
                const promises = [];
                if (data.inventory) promises.push(DB.write('inventory', arrayToObject(data.inventory)));
                if (data.suppliers) promises.push(DB.write('suppliers', arrayToObject(data.suppliers)));
                if (data.transactions) promises.push(DB.write('transactions', arrayToObject(data.transactions)));
                if (data.recipes) promises.push(DB.write('recipes', arrayToObject(data.recipes)));
                if (data.settings) promises.push(DB.settings.save(data.settings));
