...
const RecipesModule = (function() {
    ...
    function render(search) {
        ...
        container.innerHTML = filtered.map(r => {
            ...
            if (Auth.can('canEditRecipe')) {
                actions += '<button class="btn-icon" onclick="RecipesModule.editRecipe(\'' + (r._key || r.id) + '\')" title="Edit recipe"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>';
            }
            if (Auth.can('canDeleteRecipe')) {
                actions += '<button class="btn-icon" onclick="RecipesModule.confirmDelete(\'' + (r._key || r.id) + '\', \'' + app.escapeHtml(r.name) + '\')" title="Delete recipe"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>';
            }

            return '<div class="recipe-card" onclick="RecipesModule.viewRecipe(\'' + (r._key || r.id) + '\')">' +
                '<div class="recipe-card-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"></path></svg></div>' +
                '<div class="recipe-card-body">' +
                    ...
                    '</div>' +
                '</div>' +
                '<div class="recipe-card-actions" onclick="event.stopPropagation()">' + actions + '</div>' +
            '</div>';
        }).join('');
    }
    ...
    function viewRecipe(id) {
        ...
        document.getElementById('viewRecipeBody').innerHTML =
            '<div style="margin-bottom:12px;"><span class="badge-status badge-healthy">' + app.escapeHtml(recipe.category) + '</span></div>' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px;">' +
                (ingredients.length === 0 ? '<p style="color:var(--text-muted);font-size:13px;">No ingredients</p>' :
                    ingredients.map(ing => '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:var(--bg);border-radius:var(--radius-sm);"><span style="font-weight:500;">' + app.escapeHtml(ing.itemName) + '</span><span style="color:var(--text-light);font-size:13px;">' + ing.qty + ' ' + app.escapeHtml(ing.unit || '') + '</span></div>').join('')) +
            '</div>';
    ...
    function saveRecipe() {
        ...
        document.querySelectorAll('#recipeIngredients .recipe-ingredient-row').forEach(row => {
            ...
            const itemName = itemText.split('(')[0] ? itemText.split('(')[0].trim() : '';
    ...
    function createIngredientRowHTML(index, isFirst) {
        return `<div class="recipe-ingredient-row" data-index="${index}">
            <div class="form-group" style="flex:2;">${index === 0 ? '<label class="form-label">Item</label>' : ''}<select class="form-select recipe-ingredient-item" required><option value="">Select Item</option></select></div>
            <div class="form-group" style="flex:1;">${index === 0 ? '<label class="form-label">Qty</label>' : ''}<input type="number" class="form-input recipe-ingredient-qty" min="0.01" step="0.01" placeholder="Qty" required></div>
            <div class="form-group" style="flex:1;">${index === 0 ? '<label class="form-label">Unit</label>' : ''}<input type="text" class="form-input recipe-ingredient-unit" placeholder="Unit" readonly></div>
            <button type="button" class="btn-icon remove-ingredient" onclick="RecipesModule.removeIngredient(this)" style="${isFirst ? 'display:none;' : ''}margin-top:${index === 0 ? '22px' : '0'};"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        </div>`;
    }
