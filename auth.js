...
const Auth = (function() {
     ...
     * IMPROVED: More robust check with better error handling
     */
    function checkAdminExists() {
        // First, check the meta flag (most reliable if set)
        return DB.meta.getAdminExists().then(flagged => {
            console.log("[Auth] Admin flag check:", flagged);
            if (flagged === true) {
                return true; // Admin definitely exists
            }
            // If flag is false/not set, do a more thorough check
            return checkAdminExistsFallback();
        }).catch(err => {
            console.warn("[Auth] Meta flag check failed, falling back:", err.message);
            return checkAdminExistsFallback();
        });
    }

    /**
     * Fallback admin check - scans the users node
     */
    function checkAdminExistsFallback() {
        return DB.read('users').then(users => {
            ...
            const adminExists = userList.some(u => u && u.role === 'admin');
            console.log("[Auth] Admin exists (fallback check):", adminExists, "Total users:", userList.length);
            return adminExists;
        }).catch(err => {
            console.error("[Auth] Fallback check failed:", err.message);
            // If we get a permission error, assume admin exists (user likely isn't logged in yet)
            return err.code === 'PERMISSION_DENIED' || err.message.includes('permission') ? true : false;
        });
    }

    /**
     * Marks /meta/adminExists = true so future logged-out visits correctly
     * go to the login screen instead of "create first admin".
     */
    function _flagAdminExists() {
        return DB.meta.setAdminExists().catch(err => {
            console.warn("[Auth] Failed to set admin flag:", err.message);
        });
    }
