...
const InventoryModule = (function() {
        ...
        if (pageItems.length === 0) {
            ...
            tbody.innerHTML = pageItems.map(item => {
                ...
                if (Auth.can('canEditItem')) {
                    actions += '<button class="btn-icon" onclick="InventoryModule.editItem(\'' + (item._key || item.id) + '\')" title="Edit item"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>';
                }
                if (Auth.can('canDeleteItem')) {
                    actions += '<button class="btn-icon" onclick="InventoryModule.confirmDelete(\'' + (item._key || item.id) + '\', \'' + app.escapeHtml(item.name) + '\')" title="Delete item"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>';
                }
    ...
    function saveItem() {
        ...
        const data = { name, category, supplierId, unit, cost, reorderLevel, desc,
            updatedAt: new Date().toISOString(),
            updatedBy: Auth.getUser() ? Auth.getUser().uid : ''
        };
        ...
        if (id) {
            ...
            data.createdBy = Auth.getUser() ? Auth.getUser().uid : '';
    ...
    function addReceiveItem() {
        ...
        newRow.innerHTML = `
            <div class="form-group item-select-group">
                ...
                <select class="form-select receive-item-select" required><option value="">Select Item</option></select>
            </div>
            ...
            <button type="button" class="btn-icon remove-item" onclick="InventoryModule.removeReceiveItem(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        `;
    ...
    function submitReceive() {
        ...
        rows.forEach(row => {
            ...
            if (!itemId || !qty || qty < 1) { hasError = true; return; }
        ...
        received.forEach(r => {
            ...
            promises.push(DB.transactions.create({
                date, time: app.nowTimeStr(), type: 'Receive',
                itemId: r.itemId, itemName: item.name, qty: r.qty, unit: item.unit,
                to: r.branch, supplierId, supplierName, refNum,
                user: Auth.getDisplayName(), userId: Auth.getUser() ? Auth.getUser().uid : '',
                notes, unitCost: r.cost || item.cost || 0
            }));
        });
    ...
    function createFirstReceiveRowHTML() {
        return `<div class="receive-item-row" data-index="0">
            <div class="form-group item-select-group"><label class="form-label">Item</label><select class="form-select receive-item-select" required><option value="">Select Item</option></select></div>
            <div class="form-group qty-group"><label class="form-label">Quantity</label><input type="number" class="form-input receive-qty" min="1" placeholder="0" required></div>
            <div class="form-group price-group"><label class="form-label">Unit Cost</label><input type="number" class="form-input receive-price" min="0" step="0.01" placeholder="0.00"></div>
            <div class="form-group branch-group"><label class="form-label">Destination</label><select class="form-select receive-branch" required><option value="Warehouse">Warehouse</option><option value="Bamban">Bamban Branch</option><option value="Capas">Capas Branch</option></select></div>
            <button type="button" class="btn-icon remove-item" onclick="InventoryModule.removeReceiveItem(this)" style="display:none;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        </div>`;
    }
    ...
    function addTransferItem() {
        ...
        newRow.innerHTML = `
            <div class="form-group item-select-group"><label class="form-label">Item</label><select class="form-select transfer-item-select" required><option value="">Select Item</option>` +
                sorted.map(i => '<option value="' + (i._key || i.id) + '">' + app.escapeHtml(i.name) + '</option>').join('') +
            `</select></div>
            <div class="form-group qty-group"><label class="form-label">Quantity</label><input type="number" class="form-input transfer-qty" min="1" placeholder="0" required></div>
            <button type="button" class="btn-icon remove-item" onclick="InventoryModule.removeTransferItem(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        `;
        container.appendChild(newRow);
        if (container.querySelectorAll('.receive-item-row').length > 1) {
            container.querySelectorAll('.remove-item').forEach(btn => btn.style.display = '');
    ...
    function submitTransfer() {
        ...
        transfers.forEach(t => {
            ...
            promises.push(DB.transactions.create({
                date, time: app.nowTimeStr(), type: 'Transfer',
                itemId: t.itemId, itemName: item.name, qty: t.qty, unit: item.unit,
                from, to, refNum, user: Auth.getDisplayName(),
                userId: Auth.getUser() ? Auth.getUser().uid : '', notes, unitCost: item.cost || 0
            }));
        });
        ...
        Promise.all(promises).then(() => {
            ...
            const msg = err.aborted
                ? 'Transfer failed: insufficient quantity. Please review and try again.'
    ...
    function createFirstTransferRowHTML() {
        ...
        return `<div class="receive-item-row" data-index="0" style="grid-template-columns: 2fr 1fr auto; gap: 12px;">
            ...
            `</select></div>
            <div class="form-group qty-group"><label class="form-label">Quantity</label><input type="number" class="form-input transfer-qty" min="1" placeholder="0" required></div>
            <button type="button" class="btn-icon remove-item" onclick="InventoryModule.removeTransferItem(this)" style="display:none;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        </div>`;
    }
    ...
    function updateAdjustCurrentQty() {
        ...
        if (!item || !location) { display.value = '-'; return; }
        let qty = 0;
    ...
    function submitAdjustment() {
        ...
        const items = app.getInventoryData();
        const item = items.find(i => (i._key || i.id) === itemId);
        if (!item) { app.showToast('Item not found', 'error'); return; }
        ...
        Promise.all([
            ...
            DB.transactions.create({
                date, time: app.nowTimeStr(), type,
                itemId, itemName: item.name, qty, unit: item.unit,
                from: location, refNum, reason: notes,
                user: Auth.getDisplayName(),
                userId: Auth.getUser() ? Auth.getUser().uid : '',
                notes, unitCost: item.cost || 0
            })
        ]).then(() => {
            ...
            const msg = err.aborted
                ? 'Adjustment failed: insufficient quantity. Please review and try again.'
