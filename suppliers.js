...
const SuppliersModule = (function() {
        ...
        if (filtered.length === 0) {
            ...
            tbody.innerHTML = filtered.map(s => {
                ...
                if (Auth.can('canEditSupplier')) {
                    actions += `<button class="btn-icon" onclick="SuppliersModule.editSupplier('${s._key || s.id}')" title="Edit supplier" aria-label="Edit ${app.escapeHtml(s.name)}">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    </button>`;
                }
                if (Auth.can('canDeleteSupplier')) {
                    actions += `<button class="btn-icon" onclick="SuppliersModule.confirmDelete('${s._key || s.id}', '${app.escapeHtml(s.name)}')" title="Delete supplier" aria-label="Delete ${app.escapeHtml(s.name)}">
    ...
    function saveSupplier() {
        ...
        // Validation: supplier name must be unique
        const suppliers = app.getSuppliersData();
