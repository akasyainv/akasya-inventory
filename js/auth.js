...
const Auth = (function() {
    ...
    function loadUserProfile(uid) {
        return DB.users.getOne(uid).then(profile => {
            if (profile) {
                userProfile = profile;
                // If isActive is explicitly false, the user is deactivated
                ...
                // No profile found - check if this is the first-ever user (the creator)
                // by checking if they have the same email as the first admin
                userRole = 'viewer';
                userProfile = { uid, role: 'viewer', isActive: true };
            }
            return profile;
        }).catch(err => {
            if (err === 'Account deactivated') throw err;
            userRole = 'viewer';
            userProfile = { uid, role: 'viewer', isActive: true };
        });
    }
    ...
    function logout() {
        ...
        return auth.signOut().then(() => {
            ...
            userRole = null;
            userProfile = null;
            _initPromise = null; // Reset so next init starts fresh
        });
    }
    ...
    function checkAdminExists() {
        return DB.meta.getAdminExists().then(flagged => {
            ...
            // Legacy fallback - check users node directly
     ...
     * Uses a secondary Firebase app instance to create the user.
     */
