...
const InventoryModule = (function() {
    ...
    function submitReceive() {
        ...
        received.forEach(r => {
            ...
            // Build update for quantities
            const updates = {};
            if (r.branch === 'Warehouse') updates.qtyWarehouse = (item.qtyWarehouse || 0) + r.qty;
            else if (r.branch === 'Bamban') updates.qtyBamban = (item.qtyBamban || 0) + r.qty;
            else if (r.branch === 'Capas') updates.qtyCapas = (item.qtyCapas || 0) + r.qty;
            if (r.cost > 0) updates.cost = r.cost;
            updates.updatedAt = new Date().toISOString();

            promises.push(DB.inventory.update(r.itemId, updates));
    ...
    function submitTransfer() {
        ...
        transfers.forEach(t => {
            ...
            const updates = {};
            // Deduct from source
            if (from === 'Warehouse') updates.qtyWarehouse = (item.qtyWarehouse || 0) - t.qty;
            else if (from === 'Bamban') updates.qtyBamban = (item.qtyBamban || 0) - t.qty;
            else if (from === 'Capas') updates.qtyCapas = (item.qtyCapas || 0) - t.qty;
            // Add to destination
            if (to === 'Warehouse') updates.qtyWarehouse = (updates.qtyWarehouse || item.qtyWarehouse || 0) + t.qty;
            else if (to === 'Bamban') updates.qtyBamban = (updates.qtyBamban || item.qtyBamban || 0) + t.qty;
            else if (to === 'Capas') updates.qtyCapas = (updates.qtyCapas || item.qtyCapas || 0) + t.qty;
            updates.updatedAt = new Date().toISOString();

            promises.push(DB.inventory.update(t.itemId, updates));
        ...
        Promise.all(promises).then(() => {
            ...
            app.showToast('Error recording transfer: ' + err.message, 'error');
        });
    }
    ...
    function submitAdjustment() {
        ...
        // Deduct
        const updates = {};
        if (location === 'Warehouse') updates.qtyWarehouse = currentQty - qty;
        else if (location === 'Bamban') updates.qtyBamban = currentQty - qty;
        else if (location === 'Capas') updates.qtyCapas = currentQty - qty;
        updates.updatedAt = new Date().toISOString();

        Promise.all([
            DB.inventory.update(itemId, updates),
            ...
            app.showToast('Error recording adjustment: ' + err.message, 'error');
        });
    }
